...
TokenHistory = namedtuple("TokenHistory", ("tx_hash", "height", "conf", "timestamp", "amount", "balance",
                                           "tokens_deltas", "tokens_balances", "token_name",
                                           "ft_amount_str", "nft_amount_str", "ft_balance", "nft_balance"))
...
def get_token_transactions(wallet):
    ...
    for h in all_history:
        ...
        for category_id, category_delta in h.tokens_deltas.items():
            ...
            ft_amount_ = token_meta.format_amount(category_id, ft_amount)
            ft_amount_str = "-" if ft_amount < 0 else "+" + ft_amount_
            ...
            nft_amount_str = '{0:+d}'.format(nft_amount)
            token_name = get_token_name(category_id)
            token_h = TokenHistory(tx_hash, height, conf, timestamp, value, balance, tokens_deltas, tokens_balances,
